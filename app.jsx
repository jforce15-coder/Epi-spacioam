...
function App() {
  ...
  if (!ready) {
    ...
    inner = <VendorApp vendor={freshV} allVendors={vendors} allReps={reps} reps={reps.filter(function(r){return r.reportadoPor===freshV.email;})} props={props} company={company} schedules={schedules} hospUrlDay={hospUrlDay} hospUrlWeek={hospUrlWeek} adelantos={adelantos} onSvAdelantos={svAdelantos} onSubmit={upsert} onUpdate={upsert} onSvV={svV} onSvFeedback={function(fb){ svFeedback((feedback||[]).concat([fb])); }} onLogout={logout}/>;
  } else {
        ...
        {fReps.filter(function(r){return !r.paid&&isPayable(r,vendors);}).length>0&&(
          <div style={{background:"#EDF5EF",borderRadius:8,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            <div style={{fontSize:12.5,color:C.green,fontWeight:600}}>
              {fReps.filter(function(r){return !r.paid&&isPayable(r,vendors);}).length} trabajo{fReps.filter(function(r){return !r.paid;}).length!==1?"s":""} pendientes de pago{fVend!=="Todos"?" de "+( vMap[fVend]||fVend):""}
            </div>
          ...
          return (
            <div key={r.id} className="rh" onClick={function(){onSelect(r);}} style={{display:"grid",gridTemplateColumns:"1fr 88px 1fr 88px 108px",padding:"14px 18px",borderBottom:i<reps.length-1?"1px solid "+C.gray:"none",alignItems:"start",background:al?ALT[al].bg+"88":"#fff",cursor:"pointer",transition:"background .15s"}}>
              <div>
                ...
                <div style={{fontSize:11,color:C.earth,marginTop:3}}>{(function(){var v=(vendors||[]).find(function(x){return x.email===r.reportadoPor;});return v?vendorDisplay(v):(r.reportadoPor||"—");})()}</div>
              </div>
...
function LimpiezaTradForm({vendors,props,onSubmit,defaultVendor,onBack,onSaveFeedback}) {
  ...
  /* Fotos obligatorias — 1 por espacio (la fotografía más importante de cada uno) */
  ...
  var okCocina = !!(form.fotosMicroondas||form.fotosCafetera||form.fotosEcofiltro||form.fotosLavatrastos||form.fotosRefrigerador);
  var okSala   = !!(form.fotosTv||form.fotosSillon||form.fotosInsumos);
  ...
  function missingPhotoList(){
    ...
    Array.from({length:banos}).forEach(function(_,i){var b=form.fotosBanos[i]||{};if(!(b.ducha||b.inodoro))m.push("Baño "+(i+1));});
    if(!okCocina)m.push("Cocina");
    if(!okSala)m.push("Sala e insumos");
    return m;
  }
  ...
  return (
    <div style={{maxWidth:560,margin:"0 auto",padding:"0 0 80px",fontFamily:"Montserrat,sans-serif"}}>
      ...
      <div style={{padding:"28px 18px"}}>
        ...
        {/* STEP 5: Habitaciones */}
        {step===5&&(
          <WizStep title="Fotos de habitaciones" step={step} total={STEPS.length-2} onPrev={prev} onNext={next} canNext={okHab}>
            {!okHab&&<PhotoReqWarn msg="Falta la foto de al menos un cuarto — es obligatoria (1 por espacio) para continuar."/>}
        ...
        {step===6&&(
          <WizStep title="Fotos de baños" step={step} total={STEPS.length-2} onPrev={prev} onNext={next} canNext={okBanos}>
            {!okBanos&&<PhotoReqWarn msg="Sube al menos 1 foto por baño (ducha o inodoro) — es obligatoria para continuar."/>}
        ...
        {/* STEP 7: Cocina */}
        {step===7&&(
          <WizStep title="Cocina — Electrodomésticos" step={step} total={STEPS.length-2} onPrev={prev} onNext={next} canNext={okCocina}>
            {!okCocina&&<PhotoReqWarn msg="Sube al menos 1 foto de la cocina — es obligatoria para continuar."/>}
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {[
        ...
        {step===8&&(
          <WizStep title="Sala e Insumos de cortesía" step={step} total={STEPS.length-2} onPrev={prev} onNext={next} canNext={okSala}>
            {!okSala&&<PhotoReqWarn msg="Sube al menos 1 foto de la sala o los insumos — es obligatoria para continuar."/>}
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {[
...
/* ═══ VENDOR APP */
function VendorApp({vendor,allVendors,allReps,reps,props,company,schedules,hospUrlDay,hospUrlWeek,adelantos,onSvAdelantos,onSubmit,onUpdate,onSvV,onSvFeedback,onLogout}) {
...
var LOGO_WORDMARK = "data:image/png;base64,";
...
function AdminQAPanel({reps, vendors, onQA, onSelect, onUpdate, isAdmin, me}) {
  ...
  async function approveAll(){
    var pend = cleanReps.filter(function(r){return !r.qaStatus||r.qaStatus==="pendiente";});
    if(!pend.length) return;
    if(!window.confirm("¿Aprobar TODAS las "+pend.length+" limpiezas pendientes de revisión?\n\nCada técnico recibirá su notificación de aprobación.")) return;
    setBulkBusy(true);
  ...
  return (
    <div style={{maxWidth:700,margin:"0 auto",padding:"16px 16px 100px",fontFamily:"Montserrat,sans-serif",display:"flex",flexDirection:"column",gap:14}}>
      {/* Secciones */}
      <div style={{display:"flex",gap:8}}>
        {[["limpiezas","★ Limpiezas"],["danios","⚠ Daños"+(urgAlta>0?" · "+urgAlta:"")]].concat(isAdmin?[["resumen","Resumen técnicos"]]:[]).map(function(it){
          var s=sec===it[0];
      ...
      {/* Aprobar todas — solo admin */}
      {isAdmin&&counts.pendiente>0&&(
        <button onClick={approveAll} disabled={bulkBusy} style={{padding:"13px",borderRadius:10,border:"none",background:bulkBusy?"#9CA89F":"#3d6b52",color:"#fff",fontSize:13.5,fontWeight:700,cursor:bulkBusy?"wait":"pointer"}}>
          {bulkBusy?("Aprobando… "+bulkN+" / "+counts.pendiente):("✓ Aprobar todas las pendientes ("+counts.pendiente+")")}
        </button>
      )}
...
function TechSummary({reps, vendors}) {
  ...
  cleans.forEach(function(r){
    var e=r.reportadoPor||"?";
    if(!byTech[e])byTech[e]={email:e,total:0,apr:0,corr:[]};
    var t=byTech[e]; t.total++;
        ...
        return (
          <div key={t.email} style={{background:"#fff",borderRadius:12,border:"1px solid "+C.line,padding:"15px 16px",display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
              <div>
                <div style={{fontSize:14.5,fontWeight:700,color:C.black}}>{vMap[t.email]||t.email}</div>
...
function advanceState(adv, reps){
  ...
  var debited=Math.max(0, Math.min(monto, paidWeeks*semanal - revertido));
...
function AdvAdjustPanel({adv, st, onUpdate}) {
  ...
  function saveSemanal(){
    ...
    onUpdate({cobroSemanal:semVal, cuotas: Math.ceil(monto/semVal)});
  }
  function revertir(){
    if(revDis) return;
